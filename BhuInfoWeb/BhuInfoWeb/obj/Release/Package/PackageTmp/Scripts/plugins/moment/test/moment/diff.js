...
test("diff across DST",
    function(assert) {
        ...
        assert.equal(b.diff(a, "milliseconds", true),
            12 * 60 * 60 * 1000,
            "ms diff across DST");
        assert.equal(b.diff(a, "seconds", true),
            12 * 60 * 60,
            "second diff across DST");
        assert.equal(b.diff(a, "minutes", true),
            12 * 60,
            "minute diff across DST");
        assert.equal(b.diff(a, "hours", true),
            12,
            "hour diff across DST");
        assert.equal(b.diff(a, "days", true),
            (12 - dst.diff) / 24,
            "day diff across DST");
        equal(assert,
            b.diff(a, "weeks", true),
...
test("year diffs include dates",
    function(assert) {
        assert.ok(moment([2012, 1, 19]).diff(moment([2002, 1, 20]), "years", true) < 10,
...
test("month diffs",
    function(assert) {
        ...
        assert.equal(moment([2012, 0, 15]).diff([2012, 1, 15], "months", true),
            -1,
            "Jan 15 to Feb 15 should be 1 month");
        assert.equal(moment([2012, 0, 28]).diff([2012, 1, 28], "months", true),
            -1,
        ...
        assert.ok(-1 > moment([2012, 0, 31]).diff([2012, 2, 1], "months", true),
            "Jan 31 to Mar 1 should be more than 1 month");
        assert.ok(-30 / 28 < moment([2012, 0, 31]).diff([2012, 2, 1], "months", true),
        ...
        assert.ok(0 < moment("2014-02-01").diff(moment("2014-01-31"), "months", true),
...
test("exact month diffs",
    function(assert) {
        ...
        for (m1 = 0; m1 < 12; ++m1) {
            for (m2 = m1; m2 < 12; ++m2) {
                assert.equal(moment([2013, m2, 15]).diff(moment([2013, m1, 15]), "months", true),
                    m2 - m1,
...
test("negative zero",
    function(assert) {
        ...
        assert.ok(!isNegative(moment([2012, 0, 1]).diff(moment([2012, 0, 1]), "months")),
            "month diff on same date is zero, not -0");
        assert.ok(!isNegative(moment([2012, 0, 1]).diff(moment([2012, 0, 1]), "years")),
            "year diff on same date is zero, not -0");
        assert.ok(!isNegative(moment([2012, 0, 1]).diff(moment([2012, 0, 1]), "quarters")),
            "quarter diff on same date is zero, not -0");
        assert.ok(!isNegative(moment([2012, 0, 1]).diff(moment([2012, 0, 1, 1]), "days")),
            "days diff on same date is zero, not -0");
        assert.ok(!isNegative(moment([2012, 0, 1]).diff(moment([2012, 0, 1, 0, 1]), "hours")),
            "hour diff on same hour is zero, not -0");
        assert.ok(!isNegative(moment([2012, 0, 1]).diff(moment([2012, 0, 1, 0, 0, 1]), "minutes")),
            "minute diff on same minute is zero, not -0");
        assert.ok(!isNegative(moment([2012, 0, 1]).diff(moment([2012, 0, 1, 0, 0, 0, 1]), "seconds")),
