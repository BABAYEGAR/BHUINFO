...
test("parse",
    function(assert) {
        function equalTest(monthName, monthFormat, monthNum) {
            assert.equal(moment(monthName, monthFormat).month(),
                monthNum,
...
test("format week",
    function(assert) {
        var expected = [
            "Didòmhnaich Did Dò", "Diluain Dil Lu", "Dimàirt Dim Mà", "Diciadain Dic Ci", "Diardaoin Dia Ar",
            "Dihaoine Dih Ha", "Disathairne Dis Sa"
        ];
        for (var i = 0; i < expected.length; i++) {
...
test("from",
    function(assert) {
        ...
        assert.equal(start
            .from(moment([2007, 1, 28]).add({ m: 44 }), true),
            "44 mionaidean",
