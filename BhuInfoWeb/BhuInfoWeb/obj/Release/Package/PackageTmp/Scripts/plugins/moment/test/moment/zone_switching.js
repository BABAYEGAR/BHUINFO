...
test("local to zone, keepLocalTime = true",
    function(assert) {
        ...
        for (z = -12; z <= 14; ++z) {
            assert.equal(m.clone().zone(z * 60, true).format(fmt),
                m.format(fmt),
...
test("local to zone, keepLocalTime = false",
    function(assert) {
        ...
        // http://en.wikipedia.org/wiki/UTC-12:00
        for (z = -12; z <= 14; ++z) {
            assert.equal(m.clone().zone(z * 60).valueOf(),
                m.valueOf(),
                "local to zone(" + z + ":00) failed to keep utc time (implicit)");
            assert.equal(m.clone().zone(z * 60, false).valueOf(),
                m.valueOf(),
...
test("zone to local, keepLocalTime = false",
    function(assert) {
        ...
        for (z = -12; z <= 14; ++z) {
            ...
            assert.equal(m.clone().local(false).valueOf(),
                m.valueOf(),
                "zone(" + z + ":00) to local failed to keep utc time (explicit)");
            assert.equal(m.clone().local().valueOf(),
                m.valueOf(),
