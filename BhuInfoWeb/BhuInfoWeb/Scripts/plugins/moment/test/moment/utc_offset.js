...
test("utcOffset shorthand hours -> minutes",
    function(assert) {
        var i;
        for (i = -15; i <= 15; ++i) {
            assert.equal(moment().utcOffset(i).utcOffset(),
                i * 60,
                "" + i + " -> " + i * 60);
        }
...
test("cloning",
    function(assert) {
        assert.equal(moment().utcOffset(-120).clone().utcOffset(),
            -120,
            "explicit cloning should retain the offset");
        assert.equal(moment().utcOffset(120).clone().utcOffset(),
            120,
            "explicit cloning should retain the offset");
        assert.equal(moment(moment().utcOffset(-120)).utcOffset(),
            -120,
...
test("start of / end of",
    function(assert) {
        ...
        assert.equal(a.clone().startOf("day").hour(),
            0,
            "start of day should work on moments with utc offset");
        assert.equal(a.clone().startOf("day").minute(),
            0,
            "start of day should work on moments with utc offset");
        assert.equal(a.clone().startOf("hour").minute(),
            0,
            "start of hour should work on moments with utc offset");

        assert.equal(a.clone().endOf("day").hour(),
            23,
            "end of day should work on moments with utc offset");
        assert.equal(a.clone().endOf("day").minute(),
            59,
            "end of day should work on moments with utc offset");
        assert.equal(a.clone().endOf("hour").minute(),
            59,
