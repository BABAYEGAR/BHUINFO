...
test("library setter array of locales",
    function(assert) {
        assert.equal(moment.locale(["non-existent", "fr", "also-non-existent"]),
            "fr",
            "passing an array uses the first valid locale");
        assert.equal(moment.locale(["es", "fr", "also-non-existent"]),
            "es",
            "passing an array uses the first valid locale");
    });
...
test("library getter locale array and substrings",
    function(assert) {
        ...
        assert.equal(moment.locale(["fake-CA", "fake-MX", "fr"]), "fr", "always find something if possible");
        assert.equal(moment.locale(["fake-CA", "fake-MX", "fr-fake-fake-fake"]),
            "fr",
...
test("library ensure inheritance LT L LL LLL LLLL",
    function(assert) {
        ...
        assert.equal(moment().add(1, "days").locale(locale).calendar(),
            "nextDay -L-",
            "Should use instance locale in L formatting");
        assert.equal(moment().add(-1, "days").locale(locale).calendar(),
            "lastDay -LLL-",
            "Should use instance locale in LL formatting");
        assert.equal(moment().add(4, "days").locale(locale).calendar(),
            "nextWeek -LL-",
...
test("library localeData",
    function(assert) {
        ...
        assert.equal(moment.localeData(moment().locale("es")).months(jan),
            "enero",
...
test("instance locale method",
    function(assert) {
        ...
        assert.equal(moment([2012, 5, 6]).format("MMMM"),
            "June",
...
test("duration locale method",
    function(assert) {
        ...
        assert.equal(moment.duration({ seconds: 44 }).locale("es").humanize(),
            "unos segundos",
            "Use the instance specific locale");
        assert.equal(moment.duration({ seconds: 44 }).humanize(),
            "a few seconds",
...
test("duration deprecations",
    function(assert) {
        test.expectedDeprecations("moment().lang()");
        assert.equal(moment.duration().lang(),
...
test("from relative time future",
    function(assert) {
        ...
        assert.equal(start.from(moment([2007, 1, 28]).subtract({ s: 44 })),
            "in a few seconds",
...
test("from relative time past",
    function(assert) {
        ...
        assert.equal(start.from(moment([2007, 1, 28]).add({ s: 44 })),
            "a few seconds ago",
            "44 seconds = a few seconds");
...
test("month name callback function",
    function(assert) {
        ...
        assert.equal(moment([2011, 0, 1]).format("dd ddd dddd MMM MMMM"),
            "date date date date date",
            "format month name function should be able to access the moment object");
        assert.equal(moment([2011, 0, 2]).format("dd ddd dddd MMM MMMM"),
...
test("start/endOf week feature for first-day-is-monday locales",
    function(assert) {
        ...
        assert.equal(moment([2013, 0, 1]).endOf("week").day(),
            0,
...
test("moment().locale with missing key doesn't change locale",
    function(assert) {
        assert.equal(moment().locale("boo").localeData(),
            moment.localeData(),
...
test("moment().lang with missing key doesn't change locale",
    function(assert) {
        test.expectedDeprecations("moment().lang()");
        assert.equal(moment().lang("boo").localeData(),
            moment.localeData(),
            "preserve global locale in case of bad locale id");
    });
