...
test("h/hh with hour > 12",
    function(assert) {
        ...
        assert.ok(moment("06/20/2014 23:51 PM", "MM/DD/YYYY hh:mm A").parsingFlags().bigHour,
            "non-strict bigHour 23 for hh");
        assert.ok(!moment("06/20/2014 23:51 PM", "MM/DD/YYYY hh:mm A", true).isValid(), "validity 23 for hh");
        assert
            .ok(moment("06/20/2014 23:51 PM", "MM/DD/YYYY hh:mm A", true).parsingFlags().bigHour, "bigHour 23 for hh");
    });
...
test("string with spaceless format",
    function(assert) {
        assert.equal(moment("10Sep2001", "DDMMMYYYY").isValid(),
            true,
...
test("valid string iso 8601 + timezone",
    function(assert) {
        ...
        for (i = 0; i < tests.length; i++) {
            assert.equal(moment(tests[i]).isValid(), true, tests[i] + " should be valid in normal");
            assert.equal(moment.utc(tests[i]).isValid(), true, tests[i] + " should be valid in normal");
            assert.equal(moment(tests[i], moment.ISO_8601, true).isValid(),
                true,
                tests[i] + " should be valid in strict");
            assert.equal(moment.utc(tests[i], moment.ISO_8601, true).isValid(),
                true,
                tests[i] + " should be valid in strict");
        }
    });
